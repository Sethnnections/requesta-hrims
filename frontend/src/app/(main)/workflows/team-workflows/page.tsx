...
import { Search, Filter, Eye, Calendar, User, Clock, RefreshCw, Users } from 'lucide-react';
import { WorkflowType, WorkflowStatus } from '@/types/workflow';

export default function TeamWorkflowsPage() {
  const router = useRouter();
  const { user, canViewTeamWorkflowInstances, canViewDepartmentWorkflowInstances } = useAuth();
  ...
  useEffect(() => {
    if (canViewTeamWorkflowInstances() || canViewDepartmentWorkflowInstances()) {
      loadWorkflows();
    }
  }, []);
  ...
  const loadWorkflows = async () => {
    await getMyWorkflows({ initiatedByMe: false });
  };
  ...
  const filterWorkflows = () => {
    ...
    if (search) {
      filtered = filtered.filter(workflow =>
        ...
        workflow.initiatedByUser?.fullName?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  if (!canViewTeamWorkflowInstances && !canViewDepartmentWorkflowInstances) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="text-gray-600">You don't have permission to view team workflows</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-requesta-primary">Team Workflows</h1>
          <p className="text-gray-600">
            {canViewDepartmentWorkflowInstances() ? 'Department' : 'Team'} workflow requests
          </p>
        </div>
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search team workflows..."
                value={search}
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {filteredWorkflows.length} request{filteredWorkflows.length !== 1 ? 's' : ''} from your {canViewDepartmentWorkflowInstances() ? 'department' : 'team'}
              </CardDescription>
            </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">
                {search ? 'Try a different search' : 'No workflow requests from your team or department'}
              </p>
            </div>
          ) : (
