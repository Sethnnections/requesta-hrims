...
export function useAuth() {
  ...
  const getNavigationItems = useCallback(() => {
    ...
    if (hasPerm(PERMISSIONS.LOANS_MANAGE) || 
    ...
    if (hasPerm(PERMISSIONS.TRAVEL_MANAGE) || 
    ...
    if (hasPerm(PERMISSIONS.OVERTIME_MANAGE) || 
    ...
    if (hasPerm(PERMISSIONS.REPORTS_VIEW) ||
        hasPerm(PERMISSIONS.REPORTS_GENERATE)) {
  ...
  const getOrganizationSubItems = useCallback(() => {
    ...
    // Positions
    if (hasPerm(PERMISSIONS.POSITION_MANAGE) || 
        hasPerm(PERMISSIONS.POSITION_VIEW) ||
        hasPerm(PERMISSIONS.SYSTEM_FULL_ACCESS) ||
        hasPerm(PERMISSIONS.EMPLOYEES_MANAGE_ALL) ||
        hasPerm(PERMISSIONS.TEAM_MANAGE)) {
    ...
    // Grades
    if (hasPerm(PERMISSIONS.GRADE_MANAGE) || 
        hasPerm(PERMISSIONS.GRADE_VIEW) ||
        hasPerm(PERMISSIONS.SYSTEM_FULL_ACCESS) ||
        hasPerm(PERMISSIONS.EMPLOYEES_MANAGE_ALL) ||
    ...
    // Structure
    if (hasPerm(PERMISSIONS.DEPARTMENT_VIEW) || 
        hasPerm(PERMISSIONS.POSITION_VIEW) ||
        hasPerm(PERMISSIONS.SYSTEM_FULL_ACCESS) ||
        hasPerm(PERMISSIONS.EMPLOYEES_MANAGE_ALL) ||
        hasPerm(PERMISSIONS.TEAM_MANAGE) ||
  ...
  const loadUserProfile = useCallback(async (): Promise<UserProfile | null> => {
    ...
    try {
      setIsLoading(true)
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api/v1'}/profile/${user._id}`, {
        headers: {
  ...
  const hasOrganizationPermission = useCallback((action: 'create' | 'view' | 'edit' | 'delete', resource: 'department' | 'position' | 'grade') => {
    const permissionMap = {
      create: {
  ...
  const canAccessReports = useCallback(() => {
    return (
      hasPerm(PERMISSIONS.REPORTS_VIEW) ||
      hasPerm(PERMISSIONS.REPORTS_GENERATE)
    )
  }, [hasPerm])
  ...
  const refreshToken = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api/v1'}/auth/refresh-token`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return {
    ...
    hasRole: useCallback((role: string | string[]) => {
      if (!user) return false
